...
namespace FourierRocks
{
    static class MathLib
    {
        ...
        public static float CrossCorrelation(float[] iF1, float[] iF2)
        {
            return CrossCorrelation(iF1, iF2, 0);
        }
        ...
        public static float CalculateCorrelationPercent(float iCrossCorrelation, float iAutoCorrelation)
        {
            return CalculateCorrelationPercent(iCrossCorrelation, iAutoCorrelation, 1E-12f);
        }

        public static float CalculateCorrelationPercent(float iCrossCorrelation, float iAutoCorrelation,
            float iSensitivity)
        {
            return (float) Math.Round(100 * Math.Exp(-iSensitivity * Math.Abs(iCrossCorrelation - iAutoCorrelation)),
                2);
        }
    }
}
